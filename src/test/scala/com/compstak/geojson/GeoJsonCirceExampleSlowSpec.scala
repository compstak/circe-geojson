...
class GeoJsonCirceExampleSlowSuite extends FlatSpec with Matchers {
  ...
  buildFileAssertion[FeatureCollection[Double, FreeMap]](
    ...
    assert(result.features.forall(_.geometry.isInstanceOf[MultiLineString[Double]]), "The feature type is MultiLineString")
    assert(result.features.nonEmpty, "The feature set will be non-empty")
  }
  ...
  buildFileAssertion[FeatureCollection[Double, FreeMap]](
    ...
    assert(result.features.nonEmpty, "The feature set will be non-empty")
    assert(result.features.map(_.geometry) match {
  ...
  buildFileAssertion[MultiPolygon[Double]](
    statement = "process a submarket record",
    file = "SubmarketExample-001") { result =>
  ...
  private[this] def buildFileAssertion[A: Decoder](statement: String, file: String)
                                                  (asserting: A => Unit): Unit =
    it should s"$statement - $file" in {

      val path: Path = Paths.get(classLoader.getResource(s"geojson/$file.geojson").toURI)

      val parseTestFile: String => IO[A] = parser.parse(_)
        .fold(IO.raiseError, _.as[A].fold(IO.raiseError, IO.pure))

      val json = (readAll[IO](path, ExecutionContext.global, 100000) through utf8Decode[IO])
        .compile
        .lastOrError
        .unsafeRunSync()
